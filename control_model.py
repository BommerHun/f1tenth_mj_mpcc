"""
This module contains the class encapsulating the modified Car for the mjpc, its Trailer and Wheels.
"""
...
class Car_Control_Model(controlled_object.ControlledObject):
    """
    ...
    def get_identifier(cls) -> Optional[str]:
        return "Car_MJPC_model"

    def __init__(self, has_trailer: bool = False):
        super().__init__()
        self.update_frequency: float = Car_Control_Model.CTRL_FREQ  #: The frequency at which the update process runs.
        self.wb: float = 2*Car_Control_Model.WHEEL_Y  #: Distance between wheels in the Y direction.
        self.tw: float = 2*Car_Control_Model.WHEEL_X  #: Distance between wheels in the X direction.
        self.sensors: dict[str, np.array] = {}  #: Dictionary of sensor data.
        self.wheels: dict[str, Wheel] = {
            "front_left": Wheel(self.name+Wheel.FRONT_LEFT, pos=f"{Car_Control_Model.WHEEL_X} {Car_Control_Model.WHEEL_Y} 0"),
            "rear_left": Wheel(self.name+Wheel.REAR_LEFT, pos=f"-{Car_Control_Model.WHEEL_X} {Car_Control_Model.WHEEL_Y} 0"),
            "front_right": Wheel(self.name+Wheel.FRONT_RIGHT, pos=f"{Car_Control_Model.WHEEL_X} -{Car_Control_Model.WHEEL_Y} 0"),
            "rear_right": Wheel(self.name+Wheel.REAR_RIGHT, pos=f"-{Car_Control_Model.WHEEL_X} -{Car_Control_Model.WHEEL_Y} 0")
    ...
    def set_torques(self, d: float) -> None:
        ...
        torque = Car_Control_Model.C_M1*d - Car_Control_Model .C_M2*v - Car_Control_Model.C_M3*np.sign(v)
        for wheel in self.wheels.values():
            wheel.ctrl[0] = utils_general.clamp(torque, Car_Control_Model.MAX_TORQUE)
    ...
    def update(self) -> None:
        ...
        if self.trajectory is not None and self.controller is not None:
            ...
            self.set_steer_angles(delta_in=delta_vel)
            self.set_torques(d)
    ...
    def create_xml_element(self, pos: str, quat: str, color: str) -> dict[str, list[ET.Element]]:
        ...
        for wheel in self.wheels.values():
            ...
            ret["contact"].append(ET.Element("pair", geom1=wheel.name, geom2="ground", condim="6", friction=Wheel.FRICTION,  solimp = "0 0.99 0.01", solref = "0.02 1.1"))
        ...
        virtual_body = ET.SubElement(car, "body", name = "virtual_state")
        ET.SubElement(virtual_body, "joint", name = "virtual_hinge", type = "hinge")
        ET.SubElement(virtual_body, "geom", type = "box", size  = "0.02 0.02 0.02", pos = "0 0 0.15",contype="0" ,conaffinity="0")
        ret["actuator"].append(ET.Element("velocity", name = "virtual_input", ctrlrange = "0 10", joint = "virtual_hinge"))
